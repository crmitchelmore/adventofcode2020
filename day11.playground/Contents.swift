...
var input = rawInput.lines().map { $0.characters() }

func numOccupiedAround(x: Int, y: Int) -> Int {
  func isOccupied(pos: (x: Int, y: Int), dx: Int, dy: Int) -> Int {
    if pos.x < 0 || pos.y < 0 || pos.y >= input.count || pos.x >= input[pos.y].count {
      return 0
    }
    if input[pos.y][pos.x] == "." {
      return isOccupied(pos: (pos.x+dx, pos.y+dy) ,dx: dx, dy: dy)
    }
    return input[pos.y][pos.x] == "#" ? 1 : 0
  }
  return
    isOccupied(pos: (x-1,y-1), dx: -1, dy: -1) +
    isOccupied(pos: (x,y-1), dx: 0, dy: -1) +
    isOccupied(pos: (x+1,y-1), dx: 1, dy: -1) +
    isOccupied(pos: (x-1,y), dx: -1, dy: 0) +
    isOccupied(pos: (x+1,y), dx: 1, dy: 0) +
    isOccupied(pos: (x-1,y+1), dx: -1, dy: 1) +
    isOccupied(pos: (x,y+1), dx: 0, dy: 1) +
    isOccupied(pos: (x+1,y+1), dx: 1, dy: 1)
...
while (true) {
  let occupied: Int = input.reduce(0) {
    $0 + $1.reduce(0) {
      $0 + ($1 == "#" ? 1 : 0)
    }
  }
  ...
  input = input.enumerated().map { r in
    return r.element.enumerated().map { c -> Character in
      switch (c.element, numOccupiedAround(x: c.offset, y: r.offset)) {
      case ("L", 0):
        return  "#"
      case ("#", 5...):
        return  "L"
